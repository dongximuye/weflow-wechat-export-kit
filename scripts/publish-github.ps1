...
try {
    ...
    if ($remoteNames -notcontains 'origin') {
        & $gh repo create $RepositoryName --public --source $projectRoot --remote origin --push
        if ($LASTEXITCODE -ne 0) { throw '创建或推送 GitHub 公共仓库失败。' }
    } else {
        & git push -u origin main
        if ($LASTEXITCODE -ne 0) { throw '推送 main 分支失败。' }
    }

    & git tag -f -a $Version -m "WeFlow 微信聊天导出套件 $Version"
